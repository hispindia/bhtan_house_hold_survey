...
export function* getCurrentEvent() {
  ...
  const { year, selected6Month } = yield select((state) => state.data.tei.selectedYear);

  const currentEvent = getEventByYearAndHalt6Month(currentEvents, year, selected6Month);
...
export function* makeNewCurrentEventsWithCalculatedDataElements(currentEvents, dataValues) {
  ...
  const calculatedDataValues = Object.entries(dataValues).reduce((acc, [key, value]) => {
...
export function* makeNewCurrentEvents(dataValues) {
  ...
  const currentEventIndex = newCurrentEvents.findIndex((e) => e.event === newCurrentEvent.event);
...
export function* getLastEventsByYear(currentEvents, year) {
  ...
  const lastEvent6Month = getEventByYearAndHalt6Month(currentEvents, year, "6month");
